...
def test_head_somm(
    mocker: Any, basic_config: RunnableConfig, mock_chat_model_with_tools: Any
) -> None:
    ...
    assert (
        mock_llm.last_messages[0].content
        == basic_config["configurable"]["beverage_director_prompt"]
    )
