...
def init_model(config: RunnableConfig) -> BaseChatModel:
    ...
    if (
        ...
        logger.info(
            f"Initializing language model: {configuration.model_name} from {configuration.model_provider}"
        )
        llm = init_chat_model(
            configuration.model_name, model_provider=configuration.model_provider
        )
        ...
        logger.info(
            f"Retrieving language model from cache: {_llm_cache.model_name} from {_llm_cache.model_provider}"
        )
